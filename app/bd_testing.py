...
"химия"]
    ...
    db.session.add(User(login="timonich_login", hashed_password=User.get_hash("timonich_password"), name="Татьяна",
                        surname="Тимонич", middle_name="Васильевна",
                        role_id=db.session.query(Role).filter(Role.name == "admin").first().id))

    db.session.add(User(login="teacher1_login", hashed_password=User.get_hash("teacher1_password"), name="Иван",
                        surname="Иванов", middle_name="Иванович",
                        role_id=db.session.query(Role).filter(Role.name == "teacher").first().id))
    db.session.add(User(login="teacher2_login", hashed_password=User.get_hash("teacher2_password"), name="Пётр",
                        surname="Петров", middle_name="Пётрович",
                        role_id=db.session.query(Role).filter(Role.name == "teacher").first().id))

    db.session.add(User(login="parent1_login", hashed_password=User.get_hash("parent1_password"), name="Василий",
                        surname="Васильев", middle_name="Васильевич",
                        role_id=db.session.query(Role).filter(Role.name == "parent").first().id))
    db.session.add(User(login="parent2_login", hashed_password=User.get_hash("parent2_password"), name="Семён",
